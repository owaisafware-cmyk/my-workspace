...
def auto_migrate():
    ...
    # ── workspace_note columns ──────────────────────────────────────────────
    existing_note_cols = {c['name'] for c in inspector.get_columns('workspace_note')} \
        if 'workspace_note' in inspector.get_table_names() else set()

    note_migrations = [
        ("tags",      "ALTER TABLE workspace_note ADD COLUMN tags      VARCHAR(300) DEFAULT ''"),
        ("color",     "ALTER TABLE workspace_note ADD COLUMN color     VARCHAR(20)  DEFAULT 'default'"),
        ("link",      "ALTER TABLE workspace_note ADD COLUMN link      VARCHAR(500)"),
        ("folder_id", "ALTER TABLE workspace_note ADD COLUMN folder_id VARCHAR(50)"),
        ("pinned",    "ALTER TABLE workspace_note ADD COLUMN pinned    BOOLEAN      DEFAULT FALSE"),
    ]
    for col, sql in note_migrations:
        if col not in existing_note_cols:
            try:
                db.session.execute(text(sql))
                db.session.commit()
                print(f"[migrate] Added column workspace_note.{col}")
            except Exception as e:
                db.session.rollback()
                print(f"[migrate] Skipped workspace_note.{col}: {e}")

    # ── folder columns ──────────────────────────────────────────────────────
    existing_folder_cols = {c['name'] for c in inspector.get_columns('folder')} \
        if 'folder' in inspector.get_table_names() else set()

    folder_migrations = [
        ("color", "ALTER TABLE folder ADD COLUMN color VARCHAR(30) DEFAULT 'default'"),
    ]
    for col, sql in folder_migrations:
        if col not in existing_folder_cols:
            try:
                db.session.execute(text(sql))
                db.session.commit()
                print(f"[migrate] Added column folder.{col}")
            except Exception as e:
                db.session.rollback()
                print(f"[migrate] Skipped folder.{col}: {e}")


with app.app_context():
    db.create_all()        # creates brand-new tables if they don't exist yet
    auto_migrate()         # adds missing columns to existing tables
