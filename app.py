...
with app.app_context():
    # Create folder table first
    run_sql("CREATE TABLE IF NOT EXISTS folder (id VARCHAR(50) PRIMARY KEY, name VARCHAR(150) NOT NULL, color VARCHAR(30))")
    # Create all tables
    db.create_all()
    # Add missing columns to workspace_note safely
    run_sql("ALTER TABLE workspace_note ADD COLUMN IF NOT EXISTS tags VARCHAR(300) DEFAULT ''")
    run_sql("ALTER TABLE workspace_note ADD COLUMN IF NOT EXISTS folder_id VARCHAR(50)")
    run_sql("ALTER TABLE workspace_note ADD COLUMN IF NOT EXISTS pinned BOOLEAN DEFAULT FALSE")
    # Add missing columns to note_file safely
    run_sql("ALTER TABLE note_file ADD COLUMN IF NOT EXISTS mimetype VARCHAR(100)")
    # Seed welcome note if empty
    from sqlalchemy import text as t2
    with db.engine.connect() as conn:
        result = conn.execute(t2("SELECT COUNT(*) FROM workspace_note")).scalar()
    if result == 0:
        welcome = WorkspaceNote(
            ...
            color="indigo", tags="welcome,guide"
        )
...
body{background:linear-gradient(160deg,#07090f 0%,#0f1729 50%,#1a1040 100%);min-height:100vh;display:flex;justify-content:center;align-items:center;font-family:'Sora',sans-serif;padding:20px;overflow:hidden}
...
.moon{font-size:64px;margin-bottom:12px;display:block;animation:mp 4s ease-in-out infinite}
@keyframes mp{0%,100%{filter:drop-shadow(0 0 16px rgba(250,220,100,.4))}50%{filter:drop-shadow(0 0 32px rgba(250,220,100,.8))}}
...
.moon-icon{font-size:36px;animation:mp 4s ease-in-out infinite}
...
.search-clear{position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--text2);font-size:18px;cursor:pointer;display:none;padding:0}
...
.folder-chip{display:flex;align-items:center;gap:7px;padding:8px 16px;border-radius:14px;border:1.5px solid var(--border);background:var(--surface);cursor:pointer;font-size:13px;font-weight:600;color:var(--text);transition:.2s;text-decoration:none}
...
<div class="wrap">
  ...
  <!-- TAG FILTER -->
  {% set all_tags = [] %}
  {% for note in notes %}{% for t in (note.tags or '').split(',') if t.strip() %}{% if t.strip() not in all_tags %}{% set _ = all_tags.append(t.strip()) %}{% endif %}{% endfor %}{% endfor %}
  {% if all_tags %}
  <div class="tag-bar">
  ...
  <div class="folders-section">
    <div class="folders-header">
      ...
      <button class="folder-add-btn" onclick="toggleFF()">+ New Folder</button>
    </div>
    <div class="folder-form" id="folderForm">
      <form action="/folder" method="POST">
        ...
        <div class="brow">
          ...
          <button type="button" class="btn btn-g" onclick="toggleFF()">Cancel</button>
        </div>
      </form>
    </div>
    <div class="folders-grid">
      <a class="folder-chip {% if not active_folder %}active{% endif %}" href="/">
        🌕 All <span class="folder-count">{{ notes|length }}</span>
      </a>
      {% for folder in folders %}
      <a class="folder-chip {% if active_folder == folder.id %}active{% endif %}" href="/folder/{{ folder.id }}">
        📂 {{ folder.name }} <span class="folder-count">{{ folder.notes|length }}</span>
      </a>
      {% endfor %}
    </div>
  </div>
    ...
    <div class="card-body">
      ...
      <div id="view-{{ note.id }}">
        <div class="card-header">
          <div>
            ...
            <div class="card-meta">
              {% if note.folder %}<span class="folder-badge">📂 {{ note.folder.name }}</span>{% endif %}
              {% for t in (note.tags or '').split(',') if t.strip() %}
              <span class="tag-badge">{{ t.strip() }}</span>
              {% endfor %}
            </div>
          </div>
          <div class="card-actions">
            <button class="act" onclick="toggleEdit('{{ note.id }}')">✏️</button>
      ...
      <div id="edit-{{ note.id }}" style="display:none">
        <form action="/action" method="POST">
          ...
          <div class="frow"><label class="flabel">Tags (comma separated)</label><input type="text" name="tags" value="{{ note.tags }}" placeholder="work, personal, finance"></div>
          <div class="frow">
            <label class="flabel">Folder</label>
            <select name="folder_id">
              <option value="">— No Folder —</option>
              {% for folder in folders %}<option value="{{ folder.id }}" {% if note.folder_id == folder.id %}selected{% endif %}>📂 {{ folder.name }}</option>{% endfor %}
            </select>
          </div>
          <div class="frow">
  ...
  <div id="addCard" class="add-card" style="display:none">
    ...
    <form action="/action" method="POST" enctype="multipart/form-data">
      ...
      <div class="frow"><label class="flabel">Tags</label><input type="text" name="tags" placeholder="work, personal, finance"></div>
      <div class="frow">
        <label class="flabel">Folder</label>
        <select name="folder_id">
          <option value="">— No Folder —</option>
      ...
      <div class="frow">
        <label class="flabel">Attach Files</label>
...
function toggleEdit(id){const v=document.getElementById('view-'+id),e=document.getElementById('edit-'+id);v.style.display=v.style.display==='none'?'block':'none';e.style.display=e.style.display==='none'?'block':'none';}
...
function pickC(cid,name,dot){document.querySelectorAll('#'+cid+' .cdot').forEach(d=>d.classList.remove('active'));dot.classList.add('active');const inp=document.getElementById(cid.replace('cpe-','ce-').replace('cpa','ca'));if(inp)inp.value=name;}
function prevFiles(input,id){const prev=document.getElementById('prev-'+id);prev.innerHTML='';const ub=document.getElementById('upbtn-'+id);if(ub&&input.files.length)ub.style.display='block';Array.from(input.files).forEach(f=>{const c=document.createElement('div');c.className='dz-chip';c.innerHTML=(f.type.startsWith('image')?'🖼️':'📎')+' '+f.name;prev.appendChild(c);});const dz=document.getElementById('dz-'+id);if(dz&&input.files.length)dz.style.borderColor='var(--accent)';}
document.querySelectorAll('.drop-zone').forEach(z=>{z.addEventListener('dragover',e=>{e.preventDefault();z.classList.add('dragover');});z.addEventListener('dragleave',()=>z.classList.remove('dragover'));z.addEventListener('drop',e=>{e.preventDefault();z.classList.remove('dragover');const inp=z.querySelector('input[type=file]');if(inp){inp.files=e.dataTransfer.files;inp.dispatchEvent(new Event('change'));}});});
function toggleFF(){document.getElementById('folderForm').classList.toggle('open');}
let activeTag='';
function doSearch(q){q=q.trim().toLowerCase();document.getElementById('searchClear').style.display=q?'block':'none';filterCards(q,activeTag);}
function clearSearch(){document.getElementById('searchInput').value='';document.getElementById('searchClear').style.display='none';filterCards('',activeTag);}
function filterTag(tag,el){document.querySelectorAll('.tag-pill').forEach(p=>p.classList.remove('active'));el.classList.add('active');activeTag=tag;filterCards(document.getElementById('searchInput').value.trim().toLowerCase(),tag);}
function filterCards(q,tag){let v=0;document.querySelectorAll('.card').forEach(card=>{const title=card.dataset.title||'',content=card.dataset.content||'',tags=card.dataset.tags||'';const mQ=!q||(title.includes(q)||content.includes(q)||tags.includes(q));const mT=!tag||tags.split(',').map(t=>t.trim()).includes(tag);const show=mQ&&mT;card.dataset.hidden=show?'false':'true';if(show)v++;});document.getElementById('noResults').style.display=v===0?'block':'none';}
...
def get_ctx(active_folder=None):
    folders = Folder.query.all()
    notes = WorkspaceNote.query.filter_by(folder_id=active_folder).all() if active_folder else WorkspaceNote.query.all()
...
def index():
    ...
    return render_template_string(HTML_TEMPLATE, **get_ctx())

@app.route('/folder/<folder_id>')
def view_folder(folder_id):
    if not session.get('authenticated'):
        return redirect(url_for('index'))
    return render_template_string(HTML_TEMPLATE, **get_ctx(active_folder=folder_id))
...
def handle_action():
    ...
    if action == 'add':
        nid = str(uuid.uuid4())
        note = WorkspaceNote(id=nid, title=request.form.get('title','Untitled'),
            content=request.form.get('content',''), link=request.form.get('link',''),
            color=request.form.get('color','default'), tags=request.form.get('tags',''),
            folder_id=request.form.get('folder_id') or None)
        db.session.add(note); db.session.flush()
        save_files(request.files.getlist('files'), nid)
        db.session.commit()
    elif action == 'edit':
        note = WorkspaceNote.query.get(request.form.get('note_id'))
        if note:
            note.title=request.form.get('title',note.title); note.content=request.form.get('content',note.content)
            note.link=request.form.get('link',note.link); note.color=request.form.get('color',note.color)
            note.tags=request.form.get('tags',note.tags); note.folder_id=request.form.get('folder_id') or None
            db.session.commit()
    elif action == 'delete':
        note = WorkspaceNote.query.get(request.form.get('note_id'))
        if note:
        ...
        note = WorkspaceNote.query.get(request.form.get('note_id'))
        if note:
            save_files(request.files.getlist('files'), note.id); db.session.commit()
    elif action == 'delete_file':
        nf = NoteFile.query.get(request.form.get('file_id'))
        if nf:
            fp=os.path.join(app.config['UPLOAD_FOLDER'],nf.filename)
            if os.path.exists(fp): os.remove(fp)
            db.session.delete(nf); db.session.commit()
    return redirect(url_for('index'))
